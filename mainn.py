...
<TestScreen1>:
    ...
    MDBoxLayout:
        ...
        MDRaisedButton:
            ...
            stored_value:''  
            on_release:
                app.show_next_screen(self.stored_value)
        MDRaisedButton:
            text: '8'
            size_hint: None, None
            size: dp(200), dp(50)
...
class MyApp(MDApp):
    def build(self):
        ...
        self.load_model()  # Load your model and label encoder
    ...
    def show_next_screen(self,value):
        ...
        if current_screen_index < 38:  # Adjust this number based on total screens
            ...
            print(f"Button pressed with value: {value}")
    ...
    def show_prediction_screen(self):
        stored_values = self.get_stored_values()
        if stored_values:
            # Perform prediction using stored values
            prediction = self.make_prediction(stored_values)
            print(f"Prediction: {prediction}")
            # Optionally, switch to prediction screen
            self.root.current = 'prediction_screen'
        else:
            print("No stored values to make prediction.")

    def get_stored_values(self):
        # Implement logic to retrieve and return stored values from different screens
        # For demonstration, let's assume we store values in a list
        stored_values = []
        for i in range(1, 39):  # Assuming screens from test_screen_1 to test_screen_38
            screen_name = f"test_screen_{i}"
            screen = self.root.ids.screen_manager.get_screen(screen_name)
            if screen:
                # Example: Assume each screen has a stored_value attribute
                stored_value = screen.stored_value
    ...
    def make_prediction(self, values):
        # Example prediction logic using a dummy model
        # Modify this according to your actual model and data
        # For demonstration, we use a simple logic based on the sum of values
        prediction_input = np.array([values])  # Adjust input format as per your model's requirements
        prediction = self.model.predict(prediction_input)
        # Example: Convert prediction index to label using label encoder
        predicted_label = np.argmax(prediction, axis=1)
        predicted_class = self.label_encoder.inverse_transform(predicted_label)
