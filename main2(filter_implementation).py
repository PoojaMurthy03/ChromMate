        ...
        MDNavigationDrawer:
            ...
            ScrollView:
                MDBoxLayout:
                    ...
                    MDRaisedButton:
                        text: 'Color Blind Filters'
                        size_hint_x: None
                        width: root.width - dp(100)
                        on_release:
                            nav_drawer.set_state("close")
                            app.show_filter_screen()
...
<FilterScreen>:
    ...
    MDBoxLayout:
        ...
        MDBoxLayout:
            ...
            spacing: dp(20)
            MDLabel:
                text: 'Select the type of color blindness filter to apply:'
                halign: 'center'
            ...
            MDRaisedButton:
                ...
                size_hint: None, None
                size: dp(200), dp(50)
                on_release: app.show_camera_screen('deuteranopia')
            MDRaisedButton:
                text: 'Protanopia'
                pos_hint: {'center_x': 0.5}
                size_hint: None, None
                size: dp(200), dp(50)
                on_release: app.show_camera_screen('protanopia')
            MDRaisedButton:
                text: 'Tritanopia'
                pos_hint: {'center_x': 0.5}
                size_hint: None, None
                size: dp(200), dp(50)
                on_release: app.show_camera_screen('tritanopia')
...
class CameraScreen(Screen):
    ...
    def start_camera(self, filter_type):
    ...
    def apply_color_filter(self, frame):
        ...
        matrix_deuteranopia = np.array([
            [0.625, 0.7, 0],
            [0.375, 0.3, 0.3],
            [0, 0, 0.7]
        ])
        matrix_protanopia = np.array([
            [0.56667, 0.55833, 0],
            [0.43333, 0.44167, 0.24167],
            [0, 0, 0.75833]
        ])
        matrix_tritanopia = np.array([
            [0.95, 0, 0],
            [0.05, 0.43333, 0.475],
            [0, 0.56667, 0.525]
        ])
    ...
    def apply_color_filter(self, frame):
        if self.filter_type == 'deuteranopia':
            frame = cv2.applyColorMap(frame, cv2.COLORMAP_HOT)
        elif self.filter_type == 'protanopia':
            frame = cv2.applyColorMap(frame, cv2.COLORMAP_OCEAN)
        elif self.filter_type == 'tritanopia':
...
class MyApp(MDApp):
    ...
    def show_filter_screen(self):
        self.root.current = 'filter_screen'

    def show_camera_screen(self, filter_type):
        camera_screen = self.root.get_screen('camera_screen')
        camera_screen.start_camera(filter_type)
